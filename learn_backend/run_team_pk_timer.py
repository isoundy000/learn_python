...
# 需要更新的配置，需要写死
CONFIG_NAME = ['server_pk_time', 'server_pk_world', 'server_pk_reward', 'server_pk_exchange', 'team_pk_exchange']
...
TIMER_GAME_CONFIG = (
    # (game_config配置名, 处理类)
    ('server_pk_time', TimerTaskTeamPkNewDoGroup),
    ('server_pk_time', TimerTaskTeamPkNewSelectPlayer),
    ('server_pk_time', TimerTaskTeamPkNew),
    ('server_pk_time', TimerTaskTeamPkNewGiftAndScore),
)
...
# 每个服务器集群分别记录Timer
timer_model = Timer.get(uid='team_pk_timer_%s' % CONFIG_TYPE, server_name='master')
                ...
                if now >= int(task_obj.get_next_time()):
                    try:
                        if task_obj.is_global() == 1:
                            ...
                            world_ids = list(set([value['world_id'] for value in game_config.server_pk_world.itervalues()]))
