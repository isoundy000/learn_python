...
class TimerTaskTeamPkNewDoGroup(object):
    def __init__(self, config_id, version):
        self.cur_config_id = config_id
        self.key = 'server_pk_do_group_%s' % min(game_config.server_pk_time) if game_config.server_pk_time else 0
    ...
    def parser(self):
        ...
        if module:
            func = getattr(module, 'do_group')
            if func:
                self.func = func

        date = datetime.datetime.now()

        select_open_time = config['select_open_time']
        ...
        for week in config['time_sort']:
            ...
            if diff_day < 0:
                ...
                break
            elif diff_day == 0 and date.strftime("%H:%M") < select_open_time:
                func_date = date.date()
                break
        ...
        func_time_str = '%s %s' % (func_date.strftime('%Y-%m-%d'), select_open_time)
