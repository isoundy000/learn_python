...
class TimerTaskTeamPkNewSelectPlayer(object):
    def __init__(self, config_id, version):
        self.cur_config_id = config_id
        self.key = 'server_pk_select_player_%s' % min(game_config.server_pk_time) if game_config.server_pk_time else 0
    ...
    def parser(self):
        ...
        if module:
            func = getattr(module, 'select_player')
            if func:
                self.func = func

        date = datetime.datetime.now()

        select_close_time = config['select_close_time']
        ...
        for week in config['time_sort']:
            ...
            if diff_day < 0:
                ...
                break
            elif diff_day == 0 and date.strftime("%H:%M") < select_close_time:
                func_date = date.date()
                break
        ...
        func_time_str = '%s %s' % (func_date.strftime('%Y-%m-%d'), select_close_time)
