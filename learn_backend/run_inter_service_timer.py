...
settings.ENVPROCS = 'inter_service'
...
from run_timer_script.timer_task_large_super_rich_step_reward import TimerTaskLargeSuperRichStepReward

# 需要更新的配置，需要写死
CONFIG_NAME = ['large_super_rich', 'large_super_all', 'large_super_rich_all', 'server_link',
               'large_roulette', 'large_roulette_reward', 'large_pond']
...
TIMER_GAME_CONFIG = (
    # (game_config配置名, 处理类)
    ('large_super_rich_mapping', TimerTaskLargeSuperRichStepReward),
)
...
# 注意, 正式环境禁止启动此函数
def debug_sync_change_time(time_pool):
    """
    ...
    if sys_time != int(time.time()):
        ...
        reload_all_config(time_pool, force_reload=True)
        reload_all_timers(time_pool)
...
# 每个服务器集群分别记录Timer
timer_model = Timer.get(uid='run_inter_service_%s' % CONFIG_TYPE, server_name='master')
                    ...
                    try:
                        if task_obj.is_global() == 1:
                            ...
                            for world_id in world_ids:
                                print_log('world func: ', task_obj.get_func(), datetime.datetime.fromtimestamp(task_obj.next_time))
                            ...
                            for server_name, server_cfg in settings.SERVERS.iteritems():
                                ...
                                print_log('func: ', task_obj.get_func(), datetime.datetime.fromtimestamp(task_obj.next_time))
