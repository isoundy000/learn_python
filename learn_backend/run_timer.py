...
TIMER_CONFIG = {
    ...
    # 'refresh_guild_shop_day': (0,  1, 0, 5, 9, refresh_guild_shop_day, 1, 0),
}
...
# 每个服务器集群分别记录Timer
timer_model = Timer.get(uid='timer_%s' % CONFIG_TYPE, server_name='master')
            ...
            for k, task_obj in pool_copy.iteritems():
                if now >= int(task_obj.get_next_time()):
                    try:
                        ...
                        elif task_obj.is_global() == 3:
                            world_ids = list(set([value['world_id'] for value in game_config.league_world.itervalues()]))
                            for world_id in world_ids:
                                print_log('world func: ', task_obj.get_func(), datetime.datetime.fromtimestamp(task_obj.next_time))
                                gevent.joinall([gevent.Greenlet.spawn(task_obj.get_func(), world_id)])
                        else:
