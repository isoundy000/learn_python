...
from newfish.resource import getResourcePath
...
def replace(content, mask=u'*'):
    ...
    """
    global _newfishKf
    if _newfishKf is None:
        _initialize()
    return _newfishKf.replace(content, mask)
...
def isContains(content):
    ...
    if _newfishKf is None:
        _initialize()
    return _newfishKf.isContains(content)