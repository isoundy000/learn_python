...
class TYTable(object):
    ...
    def __init__(self, room, tableId):
        ...
        self.__state = TYTableState(self)       # TODO 需要redis化 桌子的状态
        self.__seats = TYSeatList(self)         # TODO 需要redis化 座位的数据控制
        self.__observers = TYObservers(self)    # TODO 需要redis化 桌子上的观察者的集合
        self.room.updateTableScore(self.getTableScore(), self.tableId, force=True)      # 更新桌子的评分机制的积分
    ...
    @property
    def maxSeatN(self):
        return self.config["maxSeatN"]  # 牌桌最大座位数
