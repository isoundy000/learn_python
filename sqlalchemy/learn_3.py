...
#     [{'name': `randint(1, 100)`, 'password': `randint(1, 100)`} for i in xrange(10)]
# )
# print User.__table__.insert(), User.__table__
# Session.commit()
...
Session.execute(User.__table__.insert().prefix_with('IGNORE'), {'id': 4, 'name': '4', 'password': '4'})
# 如何替换一个已有主键的记录？
...
# for i in xrange(100):
    # Session.add(User(name=str(randint(1, 100))))
# Session.flush()     # 或 session.commit()，执行完后，user 对象的 id 属性才可以访问（因为 id 是自增的）
# for k in xrange(100):
    # Session.add(Friendship(user_id1=randint(100, 120), user_id2=randint(100, 120)))
# Session.commit()
# Session.query(User).filter(User.id < 110).delete()
...
# 这种方法不需要执行“Base.get_by_id = get_by_id”之类的代码。不足之处就是 PyCharm 仍然无法找到这些方法的位置。
...
# print user2.money
# Decimal('100.00')
# print session.query(TanseferLog).count()
# 2L
