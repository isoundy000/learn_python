...
var JSON_DATA = null;
var copy_array = ["brave_map", "brave_copy", "general", "copy_monster"];
var GENERAL_JSON = {};
...
var set_map_data = function(){
    var str_html = "";
    for(var i in JSON_DATA["brave_map"]){
        str_html += "<option value='" + i + "'>" + i + "_" + JSON_DATA["brave_map"][i]["name"] + "</option>";
    }
    $("#maps").html(str_html);
    $("#maps_2").html(str_html);
    $("#maps_3").html(str_html);
    $("#maps").change();
    $("#maps_2").change();
    $("#maps_3").change();
};
...
var display_npc = function(){
    var maps = $("#maps").val();
    var point = $("#point").val();
    var power = JSON_DATA["brave_copy"][maps][point]["power"];
    $("#npc_power").html(power);

    var str_html = "";
    for(var i = 1; i <= 7; i ++){
        var p_str = JSON_DATA["brave_copy"][maps][point]["p" + i];
        if(p_str != 0){
            str_html += "<tr>";
...
$("#maps").on("change", function(e){
    ...
    var maps = $("#maps").val();
    var str_html = "";
    for(var i in JSON_DATA["brave_copy"][maps]){
         str_html += "<option value='" + i + "'>" + i + "_" + JSON_DATA["brave_copy"][maps][i]["name"] + "</option>";
    }
    $("#point").html(str_html);
    $("#point").change();
});
...
$("#point_2").on("change", function(e){
    ...
    $("#copy_power_2").html(JSON_DATA["brave_copy"][maps][point]["power"])
});
...
$("#point_3").on("change", function(e){
    ...
    $("#copy_power_3").html(JSON_DATA["brave_copy"][maps][point]["power"])
});
                    ...
                    if (data[i]["result"]["status"] == "success") {
                        ...
                        for(var s = 1; s <= 10; s ++){
                                str_html += "<div class=\"form-group\">" +
                                            "<label class=\"control-label col-md-4\">战斗回合" + s + ":</label>" +
                                            "<div class=\"col-md-4\"><span class=\"badge badge-info\">" +
...
$('#start_battle_more').on("click", function(e){
    ...
    var copy_list = [];
    for (var c in JSON_DATA["brave_copy"]){
        if (parseInt(c) >= map_2 && parseInt(c) <= map_3) {
            for (var k in JSON_DATA["brave_copy"][c]){
                if (parseInt(c) == map_2){
                ...
                if (parseInt(c) == map_3) {
                    if (parseInt(JSON_DATA["brave_copy"][c][k]["point"]) > point_3) {
                        continue;
                    }
                }
                copy_list.push(
                    {
                        "map": parseInt(c),
                        "point": parseInt(JSON_DATA["brave_copy"][c][k]["point"]),
                        "count": 1
                    }
                );
            }
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                for(var i in data){
                    ...
                    str_html += "<div class='col-md-2'><span>" + data[i]["map"] + "_" + JSON_DATA["brave_map"][data[i]["map"]]["name"] + "</span></div>";
                    str_html += "<div class='col-md-2'><span>" + data[i]["point"] + "_" + JSON_DATA["brave_copy"][data[i]["map"]][data[i]["point"]]["name"] + "</span></div>";
        ...
        xaxis: {
            ...
            tickFormatter: function(v, axis){
                ...
                if (G_COPY[v]["map"] != g_map){
                    temp = "<span style='color: red'>"  + JSON_DATA["brave_map"][G_COPY[v]["map"]]["name"] + "</span>";
                    g_map = G_COPY[v]["map"];
                }
                return temp;
            }
...
$("#chart_battle").bind("plothover", function (event, pos, item) {
    ...
    if (item) {
        if (previousPoint != item.dataIndex) {
            ...
            var dis_html = "地图:" + G_COPY[x]["map"] + "_" + JSON_DATA["brave_map"][G_COPY[x]["map"]]["name"] + "</br>" +
                "据点:" + G_COPY[x]["point"] + "_" + JSON_DATA["brave_copy"][G_COPY[x]["map"]][G_COPY[x]["point"]]["name"]+ "</br>" +
                "战斗力:" + JSON_DATA["brave_copy"][G_COPY[x]["map"]][G_COPY[x]["point"]]["power"] + "</br>" + "结果:" + temp + "</br>" +
                "回合:" + round;
