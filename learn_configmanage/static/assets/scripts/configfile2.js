...
var conf_timer = null;
var get_conf_message = function () {
    var success = function (data) {
        var timestamp = Date.parse(new Date());
        if (data != null){
            var upload_time = data["updated_time"];
            var five_minutes = 5 * 60 * 1000;
            if (data["status"] == 1 && conf_timer != null){
                clearInterval(conf_timer);
            }
            else if (data["status"] == 0 && timestamp - upload_time * 1000 <= five_minutes){
                if (conf_timer == null)
                    conf_timer = setInterval("get_conf_message()",5000);
            }
            var time_str = format(upload_time);
            $("#conf_message").html("<p>" + time_str + ":" + data["message"] + "</p>");
        }
    };
    my_ajax(true, '/config/getconfmessage', 'get', {}, true, success);
};
...
var commandValidation = function () {
    ...
    var submitFunc = function () {
        ...
        if (opt_password == "wozhendeyaozhemezuo"){
            my_ajax(true, "/config/onekeyopreate", 'get', data, true, success);
        }
        else{
...
$("#btn_package").on("click", function (e) {
    e.preventDefault();
    ctype = "package";
    $("#command_modal").modal("show");
});

$('#confirm_unlock_table').click(function () {
    var password = $('#unlock_table_password').val();
    if (password === 'jiebiaosuo'){
        my_ajax(true, "/unlock_table", 'get', {}, true, function () {

        },function () {
            $('#unlock_table_modal').modal('hide')
        });
    }else{
        $('#unlock_table_prompt').show()
    }
});

$('#unlock_table_modal').on('show.bs.modal', function () {
  $('#unlock_table_prompt').hide()
});
