...
def forEachPlusInsertProps(tablename, props):
    ...
    pvaluesstr = ["%s," % val if isinstance(val, Number) else "'%s'," % str(val).replace("'", "\\'") for val in props.values()]
...
def GetTableIncrValue(tablename):
    ...
    sql = """SELECT AUTO_INCREMENT FROM information_scheme.`TABLES` WHERE TABLE_SCHEMA='%s' AND TABLE_NAME='%s';""" (database, tablename)
