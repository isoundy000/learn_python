...
def _defer_to_worker(deliver, worker, work, *args, **kwargs):
    ...
    def container():
        try:
            ...
            log.msg("_defer_to_worker result:", result)
...
class TwistedEngine(object):
    ...
    def __init__(self, pool, dialect, url, reactor=None, create_worker=_threaded_worker, **kwargs):
    ...
    @classmethod
    def from_sqlalchemy_engine(cls, reactor, engine, create_worker=_threaded_worker):
    ...
    def _defer_to_engine(self, f, *a, **k):
        return _defer_to_worker(self._reactor.callFromThread, self._engine_worker, f, *a, **k)
    ...
    def execute(self, *args, **kwargs):
        return (self._defer_to_engine(self._engine.execute, *args, **kwargs).addCallback(TwistedResultProxy, self._defer_to_engine))

    def has_table(self, table_name, schema=None):
        return self._defer_to_engine(self._engine.has_table, table_name, schema)

    def table_names(self, schema=None, connection=None):
        if connection is not None:
            connection = connection._connection
        return self._defer_to_engine(self._engine.table_names, schema, connection)

    def connect(self):
        worker = self._create_worker()
        return (_defer_to_worker(self._reactor.callFromThread, worker, self._engine.connect).addCallback(TwistedConnection, self, worker))
...
class TwistedConnection(object):
    ...
    def _defer_to_cxn(self, f, *a, **k):
        return _defer_to_worker(self._engine._reactor.callFromThread, self._cxn_worker, f, *a, **k)

    def execute(self, *args, **kwargs):
        try:
            return (self._defer_to_cxn(self._connection.execute, *args, **kwargs).addCallback(TwistedResultProxy, self._defer_to_cxn))
        except AlreadyQuit:
            return fail(StatementError("This Connection is closed.", None, None, None))
    ...
    def begin(self, *args, **kwargs):
        return (self._defer_to_cxn(self._connection.begin, *args, **kwargs).addCallback(lambda txn: TwistedTransaction(txn, self)))
